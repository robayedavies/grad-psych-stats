...
# -- A.1.1. -- The summary shows: Time (1, 655; 2, 608); Instructions (explicit, 592; incidental, 671);
# and Orthography (absent, 631; present, 632).
...
# -- A.1.2. -- It should be surprising that we do not have equal numbers of observations.
...
# -- A.2.1. -- The estimate is: Instructionsincidental -0.05932 
...
# -- A.2.2. -- We can say, simply, that log odds that a response would be correct were 
# lower in the incidental compared to the explicit Instructions condition.
...
# contrasts(long.orth$Time)
# -- A.3.1. -- Changing the first number from 2 will result in warnings, showing that the number must
# match the number of factor levels for the factor.
# -- Q.3.2. -- Experiment: what happens if you change the base number for one of the factors?
# contrasts(long.orth$Time) <- contr.sum(2, base = 1)
# contrasts(long.orth$Time)
# -- A.3.2. -- Changing the base number changes which level gets coded as -1 vs. 1.
...
# -- A.4.1. -- The estimate is: mean_z_read   1.5090  
...
# -- A.4.2. -- We can say, simply, that log odds that a response would be correct were 
# higher for increasing values of mean_z_read.
...
# To produce and show a plot, don't do that, just adapt and use the plot functions e.g.
...
# -- Task 11 -- Now compare the models that converge: min vs. 2, min vs. 3 --
...
# -- A.5.1. -- Deleting the bobyqa requirement seems to have no impact on models that converge.
...
summary(long.orth.3.glmer.check)
# -- A.5.2. -- If we refit model 3 but with the random effects:
#   (Orthography + 1 || Participant) + 
#   
#   (Orthography + Instructions + 1 || Word),  
# -- We get:
