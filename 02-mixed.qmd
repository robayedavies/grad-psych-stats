...
Many Psychologists conduct studies where observations are properly understood to be structured in groups of some form but where, nevertheless, it is inappropriate to think of the observations as being nested [@baayen2008]. 
...
2. **concepts** -- Begin to develop an understanding of crossed random effects of subjects and stimuli.
3. **skills and concepts** -- Practice fitting linear mixed-effects models incorporating random effects of subjects and stimuli.
...
4. un analyses using linear mixed-effects models involving multiple random effects
...
If, however, you want to skip it, go to section @sec-intro-mixed-crossed-random.
...
For example, if you open the file `CP_study_word_naming_rt_180211.dat` (a `.dat` or tab delimited file) in Excel, you will see a spreadsheet that looks like @fig-CP-study-RTs.
...
Or, download the same folder by clicking on the [data-01-multilevel.zip](files/data-01-multilevel.zip) link.
...
Take another look at the data spreadsheet you saw earlier, @fig-CP-study-RTs.
...
Take a look at @fig-CP-study-RTs: we have a `-999` where we should have a RT for the response made by participant `AllanaD` to the word `broad`.
...
In `tidyverse` the functions designed to enable you to restructure data have evolved through a series of different forms.
...
Naming things well is important, as Jenny Bryan [teaches](https://speakerdeck.com/jennybc/how-to-name-files).
...
In this situation, we can say that the random effects are crossed [@baayen2008].
...
A scatterplot shows that response latencies decrease with increasing word frequency (@fig-freq-all). 
...
We can start by examining the frequency effect for each child in a grid of plots, with each plot  representing the $RT \sim frequency$ relationship for the data for a child (@fig-freqperchildtrellis).
...
@fig-freqperchildtrellis shows how, on average, more frequent words are associated with shorter reaction time, faster responses.
...
@fig-freqperchildlm presents a plot showing the estimates of the intercept and the coefficient of the effect of word frequency on reading RT, calculated separately for each child.
...
@fig-freqperchildlm presents the estimates of intercept and the frequency coefficient, calculated for each child, ordered by the size of the estimate.
...
#| fig-cap: "Estimated intercepts and frequency effect slopes (with SEs) calculated for each child analysed separately, with point estimates presented in order of size"
#| fig-alt: "The figure shows scatterplot: each point represents the English and the Portuguese scores for one child. A red line drawn through the points indicates a trend such that higher Portuguese scores are associated with higher English scores."
#| warning: false
#| message: false
#| fig-height: 5
...
We will discuss random effects covariance in @sec-intro-mixed-variance-covariance.
...
As we discuss later, in @sec-intro-mixed-variance-covariance, mixed-effects models estimate the spread in random differences.
...
1. If the model includes the random effect of \emph{participants on intercepts only} then all the slopes are the same (the lines in the figure are parallel) because this model assumes that the only differences between participants are differences in the intercepts.
...
Do this and you will find useful help, as [here](https://rstudio-pubs-static.s3.amazonaws.com/33653_57fc7b8e5d484c909b615d8633c01d51.html)
...
Lorch and Myers (1990) warn that analyzing only by-items mean RTs just assumes wrongly that *subjects are a fixed effect*.
