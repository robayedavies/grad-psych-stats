...
In this linear model, we see an approximate first answer to our research question.

Our problem is that the observations are assumed to be independent whereas the assumption of independence is questionable given the expectation that participants will differ: one participant's responses will perhaps be slower or less accurate than another, perhaps more or less affected by word frequency than another. We can examine that variation by estimating the intercept and the slope of the frequency effect separately using the data for each participant alone.
...
(I present the code for generating the plot in: `02-mixed-workbook-with-answers.R`)
...
You have seen the `lmer()` function code before but *practice makes perfect* so we shall go through the code step by step, as we did previously. This time, notice what is different versus what stays the same compared to what you saw in @sec-multi-lmer.
...
1. The output from the model summary first gives us **information about the model**.
...
2. We then see information listed under **Random effects**: this is where you can see information about the error variance terms estimated by the model.
...
3. Lastly, we see estimates of the coefficients (of the slopes) of the fixed effects.
