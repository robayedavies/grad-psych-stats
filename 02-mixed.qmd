...
::: callout-tip
We use the `read_` function to read in the data, entering two arguments inside the brackets after the function name, for example:
...
Doing dataset construction programmatically, using R functions, is generally more reliable, and faster, than doing it by hand. Researchers used to have to do this sort of thing by hand, using copying and pasting, in Excel or SPSS. Doing the process by hand takes many hours or days. And you *always* make errors.
...
If we inspect the \verb:long.all.noNAs: dataset, we can see that we have now got a tidy dataset with all the data we need for our analyses:
